...
def fetch_w1828(
    ...
    def _extract_defs(lemma: str, paras: list[str]) -> list[str]:
        ...
        for p in paras:
            ...
            if header_seen and len(p) > 15 and not re.match(r"[A-Z']+,", p):
                defs.append(p)
...
def run_pipeline(
    ...
    Example::
        ...
        for (l1, l2), s in sorted(rings.raw_edges.items(), key=lambda x: x[1], reverse=True)[:5]:
            print(f'{s:.2f}  {l1}–{l2}')
    """
    rings  = build_rings(path_wk, path_wn, path_mw, path_w1828, verbose=verbose, debug=debug)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--debug',       action='store_true', help='Save lemma_words_l1/l2 to results JSON')
    ...
    elif not (args.fetch_wk or args.fetch_wn):
        run_pipeline(args.path_wk, args.path_wn, args.path_mw, args.path_w1828,
                     debug=args.debug, out=args.out)
